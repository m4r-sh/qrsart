...
import { QR, createQR } from '../../src';

describe('QR Serialization', () => {
  test('Generic', () => {
    const qr = createQR({ data: 'Some data', version: 5 })

    let qr2 = QR.fromString(qr.toString())
  ...
  test('fromBytes() reconstructs QR correctly', () => {
    const bytes = new Uint8Array([3, (2 << 3) | (4 << 5), 1, 2, 3]);
    const qr = QR.fromBytes(bytes);
  ...
  test('fromString() reconstructs QR from base64', () => {
    // Create a test QR first
    const original = new QR({
      version: 1,
      ecl: 3,
      mask: 7,
  ...
  test('toBytes() and fromBytes() round trip', () => {
    ...
    const reconstructed = QR.fromBytes(bytes);
  ...
  test('toString() and fromString() round trip', () => {
    ...
    const reconstructed = QR.fromString(str);
